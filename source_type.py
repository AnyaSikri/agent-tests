...
def write_llm_source_type_to_csv(results: list[dict], filename: str = "llm_source_type.csv") -> None:
    """
    Writes the LLM source type results to a CSV file.
    Args:
        results (list[dict]): List of LLM source type info dicts.
        filename (str, optional): Output CSV filename. Defaults to 'llm_source_type.csv'.
    Returns:
        None
    """
    if not results:
...
if __name__ == "__main__":
    ...
    # Example: get all model names
    llm_names = [m.get("Model name", "Unknown") for m in models]
    ...
    print(f"Wrote LLM source type info for {len(llm_source_type_results)} models to llm_source_type.csv")
